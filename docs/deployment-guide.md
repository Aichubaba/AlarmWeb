...
Эндпоинт `/health` возвращает `200 OK` при нормальной работе приложения. Может использоваться для балансировщиков нагрузки и Kubernetes probes.

---

## 6. Метрики

Эндпоинт `/metrics` возвращает актуальные метрики приложения (Prometheus). Метрики автоматически собираются через `app.UseHttpMetrics()` и `app.MapMetrics()`.
